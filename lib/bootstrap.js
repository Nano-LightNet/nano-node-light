  ...
  handleMsg(data) {
    ...
    for (;;) {
      if (state.bulkEntry) {
        ...
        if (state.entrySize === this.entryFlag.ENTRY_SIZE) {
          if (state.bulkEntry.equals(this.entryFlag.NULL_ENTRY)) {
            ...
            const Amount = (SegmentOne << 64n) + SegmentTwo
            this.emit('entry', { Hash, Amount })
          }
          state.bulkSize = 0
        }
        ...
        if (state.frontierSize === 48) {
          ...
          const SegmentOne = state.front.readBigUInt64BE(32)
          const SegmentTwo = state.front.readBigUInt64BE(40)
