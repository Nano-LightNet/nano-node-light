...
function logElection(label, election, blockHash) {
  ...
  for (const voter of election.voters) {
    voters.push(encodeAddress({publicKey:Buffer.from(voter, "binary")}))
  }
  ...
  //console.log(label, {voters, votingWeight: rawToXNO(election.votingWeight), started, confirmed, state: STATE_NAMES[getState(election)]})
  console.log(
    label + "\n",
